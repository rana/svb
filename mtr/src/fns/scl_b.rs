...
pub fn enc(decs: &[u32]) -> Result<Vec<u8>> {
    ...
    let (hdrs, dat_len) = hdrs_and_dat_len(decs);
    let ret = vec![0u8; cnt_len + hdrs.len() + dat_len];
    // println!("hdrs:{:?}", &hdrs);
    ...
    for dec in decs.iter() {
        ...
        // let (mut hdrs2, _) = hdrs_and_dat_len(decs);
        // let mut hdrs2 = hdrs2.as_mut_slice();
        if hdr_shf_len == 8 {
            // hdrs2 = &mut hdrs2[1..];
            hdr_shf_len = 0;
            hdr_idx += 1;
